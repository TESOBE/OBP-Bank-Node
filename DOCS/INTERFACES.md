...
| **B** | Get the outbound payment onto the Open Corridor network | **Submits** Transaction Request to the OBP API (TESOBE-hosted); **fetches** counterparty state | HTTPS + OAuth2 | A payment from A1 has been persisted to the outbox | OBP OPEN_CORRIDOR Transaction Request payload, status queries |
